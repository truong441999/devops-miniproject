  ...
  stages{ 
    stage('Clone repository') {
        steps{
            ...
            url: 'https://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@github.com/truong441999/devops-miniproject.git'
        }
    }
    ...
    stage("Build Docker Image PROD ENV"){
        steps{
            // Build Image 
            sh "git branch"
            sh "docker build -f ./server/Dockerfile ./server -t truong441999/devops-server:latest"
            sh "docker build -f ./server/Dockerfile ./server -t truong441999/devops-server:${PULL_REQUEST_ID}"
            
            // Push Image to Docker Hub
            sh "echo ${DOCKERHUB_CREDENTIALS_PSW} | docker login -u ${DOCKERHUB_CREDENTIALS_USR} --password-stdin"
            sh 'docker image push truong441999/devops-server:latest'
            sh 'docker image push truong441999/devops-server:${PULL_REQUEST_ID}'
    ...
    stage("Update Deployment Manifest PROD ENV"){
        steps{
            ...
            sh '''sed -i "s+truong441999/devops-server.*+truong441999/devops-server:${PULL_REQUEST_ID}+g" ./deployment_manifest/prod/server-deployment.yaml'''
            sh "cat ./deployment_manifest/prod/server-deployment.yaml"
            sh "git add ."
